    ...
    const fetchProducts = async () => {
      try {
        ...
        const filtered = data.products.filter(
          (p: Product) => p.category === "essentials"
        );
  ...
  return (
    <div className="w-full flex flex-col items-center pb-5">
      ...
      <div className="h-16 w-full px-4 flex items-center justify-between border-y-2 border-gray-200 bg-white backdrop-blur-lg">
        {/* <button className="bg-transparent text-black text-base font-medium px-2 py-2 rounded-md uppercase">
          <Link href="/complimentary">
            <span className="text-base font-medium ">Complimentary</span>
          </Link>
        </button>
        <button className="bg-[#ff493d] text-white text-base font-medium px-2 py-2 rounded-md uppercase">
          <Link href="/shop">
        ...
        <Link href="/shop">
          <span className="text-base font-medium text-red-600">Essentials</span>
        </Link>
        <Link href="/restaurant">
          <span className="text-base font-medium">Restaurant</span>
        </Link>
      ...
      <div className="w-[95%] mt-4 mb-2 flex items-center justify-between">
        ...
        <button className="bg-black text-white text-base font-medium w-[49%] px-4 py-2 rounded-md uppercase">
          <Link href="/restaurant">Next</Link>
        </button>
      </div>
    </div>
  );
